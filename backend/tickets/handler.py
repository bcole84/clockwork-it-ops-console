...
ALLOWED_CATEGORIES = {"Account/Access", "Hardware", "Network", "Software", "Other"}
ALLOWED_STATUSES = {"New", "In Progress", "Waiting", "Resolved"}
    ...
    try:
        ...
        if method == "POST":
            ...
            item = {
                ...
                "priority": body.get("priority") if body.get("priority") in ALLOWED_PRIORITIES else "Medium",
                "category": body.get("category") if body.get("category") in ALLOWED_CATEGORIES else "Other",
                "status": "New",
        ...
        if method == "PUT" and item_id:
            ...
            body = json.loads(event.get("body") or "{}")
            for field in ("subject", "description", "requester", "notes"):
                if field in body:
                    existing[field] = body[field]
            if body.get("priority") in ALLOWED_PRIORITIES:
                existing["priority"] = body["priority"]
            if body.get("category") in ALLOWED_CATEGORIES:
